function flin_n ( n, jsearch, l, f_n, x, nf, v, q0, q1, qd0, qd1, qa, qb, qc )

!*****************************************************************************80
!
!! flin_n is the function of one variable to be minimized by minny_n.
!
!  Discussion:
!
!    f_n(X) is a scalar function of a vector argument X.
!
!    A minimizer of f_n(X) is sought along a line or parabola.
!
...
!    point at which f_n is to be evaluated.  
...
!
!    Input, external f_n, is the name of the function to be minimized.
...
!
!    Output, real ( kind = 8 ) flin_n, the value of the function at the 
!    minimizing point.
!
  implicit none
...
subroutine minny_n ( n, jsearch, nits, d2, x1, f1, fk, f_n, x, t, h, v, q0, q1, &
  nl, nf, dmin, ldt, fx, qa, qb, qc, qd0, qd1 )

!*****************************************************************************80
!
!! minny_n minimizes a scalar function of N variables along a line.
!
!  Discussion:
!
!    minny_n minimizes f_n along the line from X in the direction V(*,J) unless
!    J is less than 1, when a quadratic search is made in the plane
!    defined by Q0, Q1 and X.
!
!    If FK = true, then F1 is flin_n(X1).  Otherwise X1 and F1 are ignored
...
!    the value of (1/2) times the second derivative of f_n.
!
...
! 
  ...
  if ( .not. fk .or. abs ( x1 ) < t2 ) then
    ...
    x1 = temp * t2
    f1 = flin_n ( n, jsearch, x1, f_n, x, nf, v, q0, q1, qd0, qd1, qa, qb, qc )
...
!
!  Evaluate flin_n at another point and estimate the second derivative.
!
  do
    ...
    if ( dz ) then
      ...
      f2 = flin_n ( n, jsearch, x2, f_n, x, nf, v, q0, q1, qd0, qd1, qa, qb, qc )
...
!
!  Evaluate f_n at the predicted minimum.
!
    ok = .true.
...
double precision function praxis_n ( t0, h0, n, prin, x, f_n )

!*****************************************************************************80
!
!! PRAXIS seeks an N-dimensional minimizer X of a scalar function f_n(X).
!
!  Discussion:
!
!    PRAXIS returns the minimum of the function f_n(X,N) of N variables
!    using the principal axis method.  The gradient of the function is
!    not required.
!
!    The approximating quadratic form is
!
!      Q(x') = f_n(x,n) + (1/2) * (x'-x)' * A * (x'-x)
!
...
!
!    If f_n(X) has continuous second derivatives near X0, then A will tend 
!    to the hessian of f_n at X0 as X approaches X0.
!
!    Thanks to Andreas Zuend for pointing out an error in the form of the
!    call to the routine r8mat_print_n (), 22 March 2012.
!
...
!    praxis = f_n(x) such that if X0 is the true local minimum near X, then
...
!
!    Input, external real ( kind = 8 ) f_n, is the name of the function to be
...
!
!    Local, real ( kind = 8 ) FX, the value of f_n(X,N).
!
...
!
  ...
  if ( 0 < prin ) then
    call print2_n ( n, x, prin, fx, nf, nl )
  end if
!
...
!
    ...
    call minny_n ( n, jsearch, nits, d2, s, value, fk, f_n, x, t, &
...
!
      ...
      if ( k == 2 .and. 1 < prin ) then
        call r8vec_print_n ( n, d, '  The second difference array:' )
      end if
!
...
!
        ...
        call minny_n ( n, jsearch, nits, d2, lds, value, fk, f_n, x, t, &
          h, v, q0, q1, nl, nf, dmin, ldt, fx, qa, qb, qc, qd0, qd1 )
...
!
    call quad_n ( n, f_n, x, t, h, v, q0, q1, nl, nf, dmin, ldt, fx, qf1, &
      qa, qb, qc, qd0, qd1 )
    ...
    if ( 3 < prin ) then
      call r8mat_print_n ( n, n, v, '  The new direction vectors:' )
    end if
...
!  the main loop.
!
!  Transpose V for minfit_n:
!
    v(1:n,1:n) = transpose ( v(1:n,1:n) )
!
!  Call minfit_n to find the singular value decomposition of V.
!
!  This gives the principal values and principal directions of the
!  approximating quadratic form without squaring the condition number.
!
    call minfit_n ( n, vsmall, v, d )
!
!  Unscale the axes.
!
    if ( 1.0D+00 < scbd ) then
...
!
    ...
    if ( 1 < prin ) then
      ...
      if ( 1.0D+00 < scbd ) then
        call r8vec_print_n ( n, z, '  The scale factors:' )
      end if 

      call r8vec_print_n ( n, d, '  Principal values of the quadratic form:' )

    end if

    if ( 3 < prin ) then
      call r8mat_print_n ( n, n, v, '  The principal axes:' )
    end if
!
...
function r8_hypot_n ( x, y )

!*****************************************************************************80
!
!! r8_hypot_n returns the value of sqrt ( X^2 + Y^2).
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
!    Output, real ( kind = 8 ) r8_hypot_n, the value of sqrt ( X^2 + Y^2 ).
!
  implicit none
...
function r8_uniform_01_n ( seed )

!*****************************************************************************80
!
!! r8_uniform_01_n returns a unit pseudorandom R8.
!
!  Discussion:
!
...
!
!    Output, real ( kind = 8 ) r8_uniform_01_n, a new pseudorandom variate,
...
!
  ...
  if ( seed == 0 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'r8_uniform_01_n - Fatal error!'
  ...
  r8_uniform_01_n = real ( seed, kind = 8 ) * 4.656612875D-10
...
subroutine r8vec_print_n ( n, a, title )

!*****************************************************************************80
!
!! r8vec_print_n prints an R8VEC.
!
!  Discussion:
!
...
subroutine timestamp_n ( )

!*****************************************************************************80
!
!! timestamp_n prints the current YMDHMS date as a time stamp.
!
!  Example:
!
