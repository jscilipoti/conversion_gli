      doubleprecision FUNCTION PRAXIS_main(T0,MACHEP,H0,N,PRIN,X,f_main,
     *FMIN)

c*********************************************************************72
C                             LAST MODIFIED 3/1/73
      double precision T0,MACHEP,H0,X(N),f_main,FMIN
      EXTERNAL f_main
      INTEGER PRIN,IDIM,NMAX
      integer::loop
      PARAMETER (NMAX=40)
C
C     PRAXIS RETURNS THE MINIMUM OF THE FUNCTION f_main(X,N) OF N VARIABLES
...
C     T0       IS A TOLERANCE.  PRAXIS ATTEMPTS TO RETURN PRAXIS=f_main(X)
...
C              IF PRIN=1, f_main IS PRINTED AFTER EVERY N+1 OR N+2 LINEAR
...
C     f_main(X,N)   IS THE FUNCTION TO BE MINIMIZED.  f_main SHOULD BE A double precision
...
C              Q(X') = f_main(X,N) + (1/2) * (X'-X)-TRANSPOSE * A * (X'-X)
...
C     OF SECOND DIFFERENCES).  IF f_main HAS CONTINUOUS SECOND DERIVATIVES
C     NEAR X0, A WILL TEND TO THE HESSIAN OF f_main AT X0 AS X APPROACHES X0.
C
...
C
      ...
      double precision random_main,DSQRT,DABS
C
...
C     IN THE DIMENSION STATEMENTS IN SUBROUTINES minfit_main,min_main,flin_main,quad_main.
C
      double precision D(N),Y(N),Z(N),Q0(NMAX),Q1(NMAX),V(NMAX,NMAX)
      COMMON /GLOBAL_main/ FX,LDT,DMIN,NF,NL
...
20         V(I,I)=1.D0
      ...
      DO 30 I=1,N
           Q0(I)=X(I)
30         Q1(I)=X(I)
      IF (PRIN.GT.0) CALL print_main(N,X,PRIN,FMIN)
C
...
C     FX MUST BE PASSED TO min_main BY VALUE.
      VALUE=FX
      CALL min_main(N,1,2,D(1),S,VALUE,.FALSE.,f_main,X,T,MACHEP,H)
...
80         KL=K
           DF=0.D0
C
C.....A random_main STEP FOLLOWS (TO AVOID RESOLUTION VALLEYS).
C     PRAXIS ASSUMES THAT random_main RETURNS A random_main NUMBER UNIFORMLY
C     DISTRIBUTED IN (0,1).
C
           IF(.NOT.ILLC) GO TO 95
                DO 90 I=1,N
                     S=(0.1D0*LDT+T2*(10**KT))*(random_main(N)-0.5D0)
                     Z(I)=S
                     DO 85 J=1,N
85                        X(J)=X(J)+S*V(J,I)
90              CONTINUE
...
95         DO 105 K2=K,N
                ...
                S=0.D0
                VALUE=FX
              CALL min_main(N,K2,2,D(K2),S,VALUE,.FALSE.,f_main,X,T,
     *MACHEP,H)
                IF (ILLC) GO TO 97
                     S=SL-FX
                     GO TO 99
...
C
           ILLC=.TRUE.
           GO TO 80
110        IF (K.EQ.2.AND.PRIN.GT.1) CALL vcprnt_main(1,D,N)
C
...
C
           ...
           S=0
           VALUE=FX
         CALL min_main(N,K2,2,D(K2),S,VALUE,.FALSE.,f_main,X,T,MACHEP,H)
120        CONTINUE
           F1=FX
           FX=SF
           LDS=0
...
C     IF NO random_main STEP WAS TAKEN, V(*,KL) IS THE "NON-CONJUGATE"
...
C
           VALUE=F1
          CALL min_main(N,K,4,D(K),LDS,VALUE,.TRUE.,f_main,X,T,MACHEP,H)
...
C
171   CALL quad_main(N,f_main,X,T,MACHEP,H)
      DN=0.D0
      DO 175 I=1,N
           D(I)=1.D0/DSQRT(D(I))
           IF (DN.LT.D(I)) DN=D(I)
175   CONTINUE
      IF (PRIN.GT.3) CALL maprnt_main(1,V,IDIM,N)
      DO 180 J=1,N
           S=D(J)/DN
           DO 180 I=1,N
180             V(I,J)=S*V(I,J)
...
C     THE MAIN LOOP.
C     FIRST TRANSPOSE V FOR minfit_main:
C
200   DO 220 I=2,N
           IM1=I-1
           DO 210 J=1,IM1
...
220   CONTINUE
C
C.....CALL minfit_main TO FIND THE SINGULAR VALUE DECOMPOSITION OF V.
...
C     NUMBER.....
C
      CALL minfit_main(IDIM,N,MACHEP,VSMALL,V,D)
C
C.....UNSCALE THE AXES.....
C
...
270   CONTINUE
C
C.....sort_main THE EIGENVALUES AND EIGENVECTORS.....
C
      CALL sort_main(IDIM,N,D,V)
      DMIN=D(N)
      IF (DMIN.LT.SMALL) DMIN=SMALL
      ILLC=.FALSE.
      IF (M2*D(1).GT.DMIN) ILLC=.TRUE.
      IF (PRIN.GT.1.AND.SCBD.GT.1.D0) CALL vcprnt_main(2,Z,N)
      IF (PRIN.GT.1) CALL vcprnt_main(3,D,N)
      IF (PRIN.GT.3) CALL maprnt_main(2,V,IDIM,N)
...
C.....RETURN.....
C
400   IF (PRIN.GT.0) CALL vcprnt_main(4,X,N)
      PRAXIS_main=FX
      RETURN
      END
      double precision FUNCTION flin_main (N,J,L,f_main,X,NF)

c*********************************************************************72
      IMPLICIT double precision (A-H,O-Z)
      double precision L,X(N)
      PARAMETER (NMAX=40)
      DIMENSION Q0(NMAX),Q1(NMAX),V(NMAX,NMAX)
C...flin_main IS THE FUNCTION OF ONE REAL VARIABLE L THAT IS MINIMIZED
C   BY THE SUBROUTINE min_main...
      COMMON /Q_main/ V,Q0,Q1,QA,QB,QC,QD0,QD1,QF1
      DIMENSION T(NMAX)
      IF (J .EQ. 0) GO TO 2
...
4     NF = NF + 1
      flin_main = f_main(T,N)
      RETURN
      END
      SUBROUTINE maprnt_main(OPTION,V,M,N)

c*********************************************************************72
      double precision V(M,N)
      INTEGER OPTION,LOW,UPP
C...THE SUBROUTINE maprnt_main PRINTS THE COLUMNS OF THE NXN MATRIX V
...
104   FORMAT (E32.14,4E25.14)
      END
      SUBROUTINE min_main(N,J,NITS,D2,X1,F1,FK,f_main,X,T,MACHEP,H)
...
c*********************************************************************72
      ...
      COMMON /GLOBAL_main/ FX,LDT,DMIN,NF,NL
     .       /Q/ V,Q0,Q1,QA,QB,QC,QD0,QD1,QF1
C...THE SUBROUTINE min_main MINIMIZES f_main FROM X IN THE DIRECTION V(*,J) UNLESS
C   J IS LESS THAN 1, WHEN A QUADRATIC SEARCH IS MADE IN THE PLANE
C   DEFINED BY Q0,Q1,X.
C   D2 IS EITHER ZERO OR AN APPROXIMATION TO HALF f_main".
...
C   FOUND.
C   IF FK=.TRUE., THEN F1 IS flin_main(X1).  OTHERWISE X1 AND F1 ARE IGNORED
...
2     IF (FK.AND.DABS(X1).GE.T2) GO TO 3
      ...
      X1=TEMP*T2
      F1 = flin_main(N,J,X1,f_main,X,NF)
3     IF (F1.GT.FM) GO TO 4
      XM = X1
      FM = F1
4     IF (.NOT.DZ) GO TO 6
C...EVALUATE flin_main AT ANOTHER POINT AND ESTIMATE THE SECOND DERIVATIVE...
      X2 = -X1
      IF (F0.GE.F1) X2 = 2.D0*X1
      F2 = flin_main(N,J,X2,f_main,X,NF)
      IF (F2.GT.FM) GO TO 5
         XM = X2
         FM = F2
...
9        X2 = -H
         GO TO 11
10       X2 = H
C...EVALUATE f_main AT THE PREDICTED MINIMUM...
11    F2 = flin_main(N,J,X2,f_main,X,NF)
...
18       X(I) = X(I) + X1*V(I,J)
      RETURN
      END
      SUBROUTINE minfit_main(M,N,MACHEP,TOL,AB,Q)
...
C...AN IMPROVED VERSION OF minfit_main (SEE GOLUB AND REINSCH, 1969)
...
1           S = S + AB(J,I)**2
         ...
         IF (S.LT.TOL) GO TO 4
            ...
            DO 3 J=L,N
               f_main = 0.D0
               DO 2 K=I,N
2                 f_main = f_main + AB(K,I)*AB(K,J)
               f_main = f_main/H
               DO 3 K=I,N
3                 AB(K,J) = AB(K,J) + f_main*AB(K,I)
4        Q(I) = G
         S = 0.D0
         IF (I.EQ.N) GO TO 6
         DO 5 J=L,N
...
110      C = 0.D0
         ...
         DO 116 I=L,K
            f_main = S*E(I)
            E(I) = C*E(I)
            IF (DABS(f_main).LE.EPS) GO TO 120
            G = Q(I)
C...Q(I) = H = DSQRT(G*G + f_main*f_main)...
            IF (DABS(f_main).LT.DABS(G)) GO TO 113
            IF (f_main) 112,111,112
111         H = 0.D0
            GO TO 114
112         H = DABS(f_main)*DSQRT(1 + (G/f_main)**2)
            GO TO 114
113         H = DABS(G)*DSQRT(1 + (f_main/G)**2)
114         Q(I) = H
...
C...SHIFT FROM BOTTOM 2*2 MINOR...
         ...
         G = E(K-1)
         H = E(K)
         f_main = ((Y - Z)*(Y + Z) + (G - H)*(G + H))/(2*H*Y)
         G = DSQRT(f_main*f_main + 1.0D0)
         TEMP = f_main - G
         IF (f_main.GE.0.D0) TEMP = f_main + G
         f_main = ((X - Z)*(X + Z) + H*(Y/TEMP - H))/X
C...NEXT QR TRANSFORMATION...
         ...
         DO 132 I=LP1,K
            ...
            H = S*G
            G = G*C
            IF (DABS(f_main).LT.DABS(H)) GO TO 123
            IF (f_main) 122,121,122
121         Z = 0.D0
            GO TO 124
122         Z = DABS(f_main)*DSQRT(1 + (H/f_main)**2)
            GO TO 124
123         Z = DABS(H)*DSQRT(1 + (f_main/H)**2)
124         E(I-1) = Z
...
125         C = f_main/Z
            ...
            DO 126 J=1,N
               ...
               AB(J,I-1) = X*C + Z*S
126            AB(J,I) = -X*S + Z*C
            IF (DABS(f_main).LT.DABS(H)) GO TO 129
            IF (f_main) 128,127,128
127         Z = 0.D0
            GO TO 130
128         Z = DABS(f_main)*DSQRT(1 + (H/f_main)**2)
            GO TO 130
129         Z = DABS(H)*DSQRT(1 + (f_main/H)**2)
130         Q(I-1) = Z
...
200   Q(1) = AB(1,1)
      ...
      RETURN
      END
      SUBROUTINE print_main(N,X,PRIN,FMIN)
...
101   FORMAT (/' AFTER',I6,
     ...
     . ' TIMES.  THE SMALLEST VALUE FOUND IS f_main(X) = ',E21.14)
102   FORMAT (' LOG (f_main(X)-',E21.14,') = ',E21.14)
103   FORMAT (' LOG (f_main(X)-',E21.14,') IS UNDEFINED.')
104   FORMAT (' X IS:',E26.14/(E32.14))
      END
      SUBROUTINE quad_main(N,f_main,X,T,MACHEP,H)

c*********************************************************************72
      IMPLICIT double precision (A-H,O-Z)
      EXTERNAL f_main
C...quad_main LOOKS FOR THE MINIMUM OF f_main ALONG A CURVE DEFINED BY Q0,Q1,X...
      PARAMETER (NMAX=40)
...
1        QD1 = QD1 + (S-L)**2
      ...
      VALUE=QF1
      CALL min_main(N,0,2,S,L,VALUE,.TRUE.,f_main,X,T,MACHEP,H)
...
4        X(I) = (QA*S + QB*X(I)) + QC*Q1(I)
      RETURN
      END
      function random_main ( naught )

c*********************************************************************72
c
cc random_main is a random_main number generator.
c
      implicit none
      ...
      ran1 = ran1 + half
      ran3(ran2) = ran1
      random_main = ran1 + .5d0

      return
      end
      SUBROUTINE sort_main(M,N,D,V)
...
3        CONTINUE
      RETURN
      END
      SUBROUTINE vcprnt_main(OPTION,V,N)
